...
const RootComponent = (props: RootComponentProps) => {
  ...
  const { data: rankData } = useSWR<responseSchema<RANKS_ITEM>>(RANKS);
  ...
  const onGotoHandler = useCallback(
    (props: PropsForAction<MERCHANTS_STORES_MEMBERSHIPS_ITEM>) => {
      return (e: React.SyntheticEvent) => {
        ...
        if (storeUrl) {
          ...
          if (id) {
            window.open(
              `/${PATHNAME.DOI_TAC}/${PATHNAME.THIET_LAP_QUAN}/${id}`,
              "_blank"
            );
          }
        }
    ...
    return (
      <Fragment>
        ...
        <Grid item xs={12}>
          ...
          {/* <CompoundTableWithFunction<MERCHANTS_STORES_MEMBERSHIPS_ITEM>
            url={defaultValues.memberships}
            passHandler={passHandler}
            columnFn={MembershipColumn}
            rankData={rankData.results}
            orderUrl={defaultValues.orders}
            onGotoHandler={onGotoHandler}
          /> */}
  ...
  }, [data, rankData, defaultValues.orders, itemCount, isLoading, pagination]);
